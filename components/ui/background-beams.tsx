...
const beamPalettes = [
    ["#d0f2a0", "#9FDE5A", "#4DD9FF"],
    ["#9FDE5A", "#4DD9FF", "#7bc234"],
    ["#4DD9FF", "#9FDE5A", "#b4e86b"],
] as const;
    ...
    return (
        <div className={cn("pointer-events-none absolute inset-0 h-full w-full overflow-hidden", className)}>
            ...
            >
                <g opacity="0.06">
                    {pathData.map((d, i) => (
                        <path key={`static-${i}`} d={d} stroke="#9FDE5A" strokeWidth="0.45" />
                    ))}
                </g>
                ...
                <defs>
                    {pathData.map((_, i) => (
                        <linearGradient
                            key={`gradient-${i}`}
                            id={`${gradientPrefix}-gradient-${i}`}
                            x1="0%"
                            y1="0%"
                            x2="100%"
                            y2="100%"
                        >
                            <stop offset="0%" stopColor={beamPalettes[i % beamPalettes.length][0]} stopOpacity="0" />
                            <stop offset="18%" stopColor={beamPalettes[i % beamPalettes.length][0]} stopOpacity="0.95" />
                            <stop offset="50%" stopColor={beamPalettes[i % beamPalettes.length][1]} stopOpacity="1" />
                            <stop offset="82%" stopColor={beamPalettes[i % beamPalettes.length][2]} stopOpacity="0.95" />
                            <stop offset="100%" stopColor={beamPalettes[i % beamPalettes.length][2]} stopOpacity="0" />
                        </linearGradient>
                    ))}
                </defs>
            </svg>
